...
학습 자체를 잘 하기 위한 학습 기법을 개발하는 데이터 증강.  
...
앞에서 언급한 Neural style transfer를 사용하는 강화학습 이미지 증강.  
동일한 클래스에서 두 이미지를 가져오고 다른 임의의 이미지로 변환. (Neural style transfer과 동일)  

새로 만들어진 이미지와 기존의 이미지간의 오차를 계산하여 이미지 분류 모델에 학습.  
해당 오차를 이용하여 Content Image로 인식이 되면서도 Style Image의 특징을 많이 넣어 새로운 이미지가 될 수 있는 최대치를 기억하고, 이를 이용하여 자동으로 이미지 증강 진행.  
서로 다른 이미지 간의 콘텐츠 및 스타일 이미지에 대한 최적의 가중치와 CNN의 이미지 간의 매핑을 학습.  
## Smart Augmentation
Neural style transfer가 아닌 cnn의 학습된 매개변수만 기존의 증강 기술에 적용한 방법. 
훈련을 위해 새로운 이미지를 생성하는 네트워크와 생성된 이미지의 오차율을 계산하는 네트워크로 총 2개의 네트워크 사용.  
위에서 Mixxing Image과 비슷하지만 영역 단위가 아니라 더 정교하게 두 이미지를 섞는다는 차별이 있음.  


## Auto Augment
강화학습 기반의 알고리즘 이미지 증강.  
컴퓨터가 반복적으로 다양한 기하학적 변환을 적용하면서 가장 효과적인 augmentation 조합을 찾는 방법.  
![image](https://user-images.githubusercontent.com/108729047/217140165-cdc4f11e-77c5-44bd-b0ef-369463c0a070.png)  
데이터의 특징을 잘 반영하는 augmentation policy를 학습.  
2개의 augmentation 조합을 찾고 해당 augmentation을 몇 퍼센트의 확률로 적용할건지 학습.  

자르기 + 뒤집기, 자르기 + 회전 조합이 가장 높은 성능을 가짐.  
